...
export const useOrbitControl = (
  ...
  const cameraToBackView = async (worldBound: WorldBound) => {
    ...
    // Đảo ngược hướng z để nhìn từ phía sau
    const z = -(isMobile ? width * 2.3 : width * 2.3) / depth * 10;

    const control: any = orbitControlRef.current;
    const camera = control.object;
    // Đảo ngược hướng z để nhìn từ phía sau
      const targetCamera = { x: 0, y: 0,  z    };
