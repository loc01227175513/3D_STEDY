...
import { useOrbitControl } from './useOrbitControlkitchenIsland';
import { useMediaQuery } from '@mui/material';
import theme from '@/themes';
import { WorldBound } from '@/types/model';
import { Vector3 } from 'three';

export const use3DScreenshotIndokitchenisland = (
  ...
  const { setCaptureBase64StringKitchenIsland } = use3DCaptureStore();
  const { setTotalDimension } = useDimensionStore();

  const fullScreenIndoKitchenIsland = () => {
  ...
  const handleScreenshotIndoKitchenIsland = async (
    ...
    if (canvas) {
      ...
      setCaptureBase64StringKitchenIsland(base64);
    }
  };
  ...
  return { fullScreenIndoKitchenIsland, handleScreenshotIndoKitchenIsland };
};
